...
def traffic_lights():
    ...
    if dropped_vehicle.is_at_traffic_light():
        ...
        if traffic_light.get_state() == carla.TrafficLightState.Yellow:
            dropped_vehicle.set_light(carla.VehicleLightState(carla.VehicleLightState.Brake | carla.VehicleLightState.LeftBlinker | carla.VehicleLightState.LowBeam))
            dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.5, steer=0.21))
            time.sleep(5)
            dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.1, steer=0.21))
            time.sleep(5)
